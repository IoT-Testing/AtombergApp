...
/**
 * MemberDevicePage â€“ Page Object operated on the MEMBER phone.
 * The member (non-admin) account verifies:
 *   â€“ Devices received via sharing appear on their home screen
 *   â€“ Controls are accessible or blocked based on their permission mask (spec Â§3)
 *   â€“ Permission-denied state is shown when the relevant bit is not set (spec Â§6.6)
 *   â€“ Joining via QR code (spec Â§4, Flow 3, step 3)
 */
public class MemberDevicePage {
     ...
     * navigate â€” the previous version tapped a positional
    ...
    public boolean returnToHomeScreen() {
        for (int attempt = 1; attempt <= 3; attempt++) {
            ...
            if (!tapOptional(HOME_TAB, "Home/dashboard tab")) {
                logpoint("[MemberDevicePage] Dashboard tab not visible (attempt "
                        + attempt + ") â€” backing out and retrying.");
     ...
     * changes made since the screen was drawn â€” chiefly access the admin revoked
     ...
     * finger DOWNWARD (y 0.5 â†’ 0.8), the pull-to-refresh direction. {@code Scroll.Up}
    ...
    public MemberDevicePage refreshHomeDashboard() {
        if (!returnToHomeScreen()) {
            System.err.println("[MemberDevicePage] Not on Home â€” skipping dashboard refresh.");
            return this;
        }
        ...
        logpoint("[MemberDevicePage] Member dashboard refreshed.");
        return this;
    }
    ...
    public boolean isOnHomeScreen() {
        ...
        logpoint("[MemberDevicePage] On home screen? " + home);
        return home;
    }
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // FAN PERMISSION ASSERTIONS (spec Â§3.1)
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    public boolean hasFullFanAccess(String deviceName) {
        ...
        logpoint("[MemberDevicePage] Fan full access â€” "
     ...
     * Control UI but NOT Edit or Analytics (spec Â§3.1 Basic Default column).
     */
    public boolean hasBasicFanAccess(String deviceName) {
        ...
        logpoint("[MemberDevicePage] Fan basic access â€” "
     ...
     * Spec Â§6.6: "Client reads bitmask, detects bit is not set, shows permission-denied state."
     */
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // QR JOIN FLOW (spec Â§4, Flow 3, step 3 â€” member side)
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
     ...
     * scanning of the QR is outside test automation scope â€” in real test
     ...
     * invite code can be entered. Path: More tab â†’ Manage family â†’ Join family.
     ...
     * phone â€” the camera-less path required because Appium cannot drive the QR
     ...
     * pre-configured permissions (spec Â§4, Flow 3, step 3 â†’ /join_family_v2).
     *
    ...
    public MemberDevicePage joinViaCode(String inviteCode) {
        if (inviteCode == null || inviteCode.isBlank())
            throw new IllegalArgumentException("Invite code is null/blank â€” admin readInviteCode() failed.");
        ...
        try {
            ...
            if (fields.size() > 1 && fields.size() >= inviteCode.length()) {
                ...
                logpoint("[MemberDevicePage] Entered invite code across "
                ...
                logpoint("[MemberDevicePage] Entered invite code: " + inviteCode);
            }
        } catch (Exception e) {
        ...
        // Poll for the confirmation straight away â€” no sleep first. "Added to home
        ...
        try {
            ...
            joinConfirmed = true;
            logpoint("[MemberDevicePage] Join confirmed: \"Added to home successfully!\"");
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // FLOW 2 (member) â€” join via Manage Family â†’ (+) Add â†’ Enter code
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    /**
     * Flow 2 member-side join: More tab â†’ Manage Family â†’ (+) Add â†’
     * "Join an existing smart home" â†’ Enter code.
     *
     * <p>The (+) FAB opens a sheet offering both "create a new home" and "join an
     * existing" â€” the join option must be picked before any code field exists. This
    ...
    public MemberDevicePage joinViaManageFamilyAddCode(String inviteCode) {
        ...
        if (!tapOptional(JOIN_EXISTING_HOME_OPTION, "Join an existing smart home")) {
            ...
            throw new RuntimeException("[MemberDevicePage] 'Join an existing smart home' not found "
                    + "on the Add-family sheet â€” see test-output/page-source/member_add_family_sheet.xml");
        }
        ActionsUtil.SSleep(2);
    ...
    public boolean badgeMatches(String deviceName, String expectedLevelLabel) {
        ...
        logpoint("[MemberDevicePage] Badge for " + deviceName + " = '" + badge
                + "' (expected contains '" + expectedLevelLabel + "') â†’ " + ok);
        return ok;
    }

    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // FLOW 2 (cleanup) â€” member leaves the joined family
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

    /**
     * Flow 2 cleanup on the member phone: More â†’ Manage Family â†’ open the joined
     * family â†’ Leave. Reuses the existing {@code MoreTab} family-management locators.
    ...
    public MemberDevicePage leaveFamily(String homeHint) {
        ...
        // test failed â€” mid-join sheet, a dialog, a device detail. Without this the
        ...
        // Open the family, then Leave â€” "Leave home" lives inside an opened family.
        ...
        // Opening the family does not surface "Leave home" â€” it sits behind the
        ...
        if (!tapOptional(LEAVE_HOME, "Leave home")) {
            if (!tapOptional(DELETE_HOME, "Delete home (fallback)")) {
                // Neither action reachable â€” the menu tap missed, or the absolute path
                ...
                System.err.println("[MemberDevicePage] Neither 'Leave home' nor 'Delete home' "
                        + "found after opening the family options menu â€” the member is still "
                        + "in '" + homeHint + "'.");
    ...
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // Lock / Water-Purifier per-capability checks â€” STUBS (presence+badge for now)
    //
    ...
    private boolean capabilityStub(String capability) {
        logpoint("[MemberDevicePage] TODO capability check not yet implemented: "
                + capability + " â€” Flow 2 relies on presence + badge for now.");
        return false;
    }

    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    // FAMILY SWITCHER â€” after joining, move to the admin's family and compare
    // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    public boolean switchToFamily(String familyName) {
        ...
        // precondition, not a nicety â€” reaching this from the More tab is what made
        // the trigger unfindable.
        if (!returnToHomeScreen()) {
            System.err.println("[MemberDevicePage] Not on Home â€” cannot open the family switcher.");
            return false;
        }

        if (isPresent(familySwitcherHeader(familyName))) {
            // Header already shows the target â€” but it is also the control that
            // OPENS the switcher, so presence alone isn't proof we're on it.
            // Opening and re-selecting is idempotent and costs one tap.
            logpoint("[MemberDevicePage] Family header already shows '" + familyName + "'.");
        }
    ...
    public java.util.List<String> listDevicesOnHome() {
        ...
        logpoint("[MemberDevicePage] Member home devices: " + AppUtil.prettyTiles(tiles));
        return tiles;
    }
    ...
    public void dumpTree(String tag) {
        try {
            ...
            logpoint("[MemberDevicePage] Page source dumped: " + file.toAbsolutePath());
        } catch (Exception ex) {
    ...
    /** Page source, or null if it could not be read â€” used for scroll change detection. */
    ...
    private void openDevice(String deviceName) {
        try {
            ...
            card.click();
            logpoint("[MemberDevicePage] Opened device: " + deviceName);
        } catch (Exception e) {
    ...
    private void tap(By locator, String label) {
        try {
            ...
            logpoint("[MemberDevicePage] Tapped: " + label);
        } catch (Exception e) {
     ...
     * when absent â€” used for optional nav steps (menus/dialogs that some app
     * versions skip) in the Flow 2 join and cleanup paths.
     */
    private boolean tapOptional(By locator, String label) {
        if (!isPresent(locator)) {
            logpoint("[MemberDevicePage] Not present (skipping): " + label);
            return false;
        }
        try {
            ...
            System.err.println("[MemberDevicePage] Optional tap failed: " + label + " â€” " + e.getMessage());
            return false;
        }
    }
