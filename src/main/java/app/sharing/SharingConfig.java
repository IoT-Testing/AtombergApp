...
/**
 * SharingConfig – single source of configuration for the Device Sharing tests.
 *
 ...
 * NOT reach a Maven/Surefire fork launched from another — a recurring source of
    ...
    private static synchronized void ensureLoaded() {
        ...
        if (Files.isReadable(p)) {
            try (InputStream in = Files.newInputStream(p)) {
                PROPS.load(in);
                System.out.println("[SharingConfig] Loaded config: " + p.toAbsolutePath());
                return;
            } catch (IOException e) {
        ...
        try (InputStream in = SharingConfig.class.getClassLoader().getResourceAsStream(FILE_NAME)) {
            if (in != null) {
                PROPS.load(in);
                System.out.println("[SharingConfig] Loaded config from classpath: " + FILE_NAME);
            } else {
                System.out.println("[SharingConfig] No " + FILE_NAME
                        + " found — falling back to environment variables / -D properties.");
            }
        } catch (IOException e) {
