 ...
 * The smart lock is Bluetooth-only and has no cloud state â€” verify it via UI
 ...
 * <ul>
 *   <li>{@code "power"} â†’ {@code power} (boolean)</li>
 *   <li>{@code "speed"} â†’ {@code last_recorded_speed} (int)</li>
 *   <li>{@code "timer"} â†’ {@code timer_hours} (int)</li>
 *   <li>{@code "led"}   â†’ {@code led} (boolean)</li>
 *   <li>{@code "sleep"} â†’ {@code sleep_mode} (boolean)</li>
 * </ul>
 */
public class DeviceStateVerifier {
    ...
    public boolean awaitState(String deviceId, String field, String expected, Duration timeout) {
        ...
        logpoint("[DeviceStateVerifier] timeout waiting for " + field
