...
/**
 * DeviceShareLongPressFlowTest â€” end-to-end, TWO-PHONE test for the "Flow 2"
 * device-sharing path:
 *
 * <pre>
 *   ADMIN : Home â†’ long-press a device tile â†’ "Share device"
 *           â†’ "Share access to family?" dialog â†’ Yes
 *           â†’ QR + invite code screen â†’ read the code into a variable
 *   MEMBER: More â†’ Manage family â†’ (+) Add â†’ enter the code â†’ Join
 *           â†’ Home â†’ Switch Family â†’ the admin's family
 ...
 * per-device matrix â€” the long-pressed tile is only the entry point, and the
...
public class DeviceShareLongPressFlowTest extends BaseDeviceSharingTest {
    ...
    /** Family/home name to open on the admin phone â€” ADMIN_FAMILY_HINT (e.g. "Main Family"). */
    ...
    /** Best-effort scrape of the admin's home tiles â€” reported, never asserted on. */
    private List<String> adminHomeDevices = new ArrayList<>();

    // â”€â”€ Pre-flight: all target devices must exist on the admin home â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    public void verifyAdminHomeHasAllDevices() {
        reporter.startTest("Flow2 pre-flight â€“ admin home has all devices", "Admin_Device");
        try {
            adminHomeDevices = adminSharingPage.listDevicesOnHome();

            // Ground truth for the tile structure â€” the scrape and the family-switcher
            ...
            Assert.assertTrue(missing.isEmpty(),
                    "Admin home is missing device tile(s): " + missing
                            + " â€” add them to the admin's family, or fix the TEST_*_DEVICE_NAME values "
            ...
            if (adminHomeDevices.isEmpty()) {
                ...
                reporter.log(Status.WARNING, "Tile scrape returned nothing â€” parity will cover "
    ...
    // â”€â”€ The round-trip â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    /**
     * Drives the whole flow end to end. The invite code is the hand-off between the
     * two phones â€” read off the admin's QR screen and held in {@code inviteCode} for
     * the member to type, because Appium cannot drive the camera to scan the QR.
     */
    @Test(description = "Long-press share â†’ Yes â†’ member joins via code â†’ switch family "
            + "â†’ both phones list the same devices")
    public void shareViaLongPress() {
        reporter.startTest("Flow2 LongPress â€“ family share round-trip", "Admin+Member");
        try {
            // â”€â”€ ADMIN: long-press the tile â†’ "Share device" â†’ confirmation â†’ Yes â”€â”€
            ...
            if (adminSharingPage.isQrCodeVisible()) {
                ...
                reporter.log(Status.WARNING, "QR image not detected â€” continuing on the invite code.");
            }

            // â”€â”€ The code: read once, stored, carried to the other phone â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            Assert.assertTrue(isSet(inviteCode),
                    "Admin must be able to read/copy the invite code from the share screen "
                            + "â€” see test-output/page-source/invite_code_not_read.xml for what was on screen.");
            reporter.log(Status.INFO, "Invite code captured: " + inviteCode);
            logpoint("[Flow2LP] Invite code = " + inviteCode);

            // â”€â”€ MEMBER: Manage Family â†’ (+) Add â†’ Join an existing smart home â”€â”€â”€â”€â”€
            //            â†’ enter code â†’ Join â†’ "Added to home successfully!"
            ...
            Assert.assertTrue(memberDevicePage.wasJoinConfirmed(),
                    "Member never saw \"Added to home successfully!\" after submitting code '"
                            + inviteCode + "' â€” the join did not land. See "
            ...
            // â”€â”€ MEMBER: Home â†’ Switch Family â†’ the admin's family â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            // The join finishes on the More tab, so landing on Home is its own step
            // and its own assertion â€” the family switcher header exists nowhere else,
            ...
            Assert.assertTrue(memberDevicePage.returnToHomeScreen(),
                    "Member phone did not return to the Home/Devices dashboard after joining "
                            + "â€” the family switcher is only reachable from there. See "
            ...
            Assert.assertTrue(memberDevicePage.switchToFamily(FAMILY_HOME_HINT),
                    "Member could not switch to '" + FAMILY_HOME_HINT + "' after joining "
                            + "â€” the join may not have landed, or the family switcher locator needs "
                            + "pinning (see the dumped page source).");

            // â”€â”€ VERIFY: both phones show the same devices â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
     ...
     * two scrapes â€” the scrape is unpinned, and a locator that already proved itself
    ...
    private void assertSameDevices() {
        ...
        logpoint("[Flow2LP] Expected on member: " + AppUtil.prettyTiles(expectedDevices));
        logpoint("[Flow2LP] Member scrape     : " + AppUtil.prettyTiles(memberScrape));
        if (!extraOnMember.isEmpty()) {
            logpoint("[Flow2LP] On member but not scraped from admin: "
                    + AppUtil.prettyTiles(extraOnMember)
                    + " â€” expected while the admin-side scrape is unpinned; not asserted.");
        }
    ...
    // â”€â”€ Revocation: two equivalent paths, one picked per run â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    /** How the member's access is revoked at the end of a run. */
    private enum RevokeMode {
        /** MEMBER: More â†’ Manage Family â†’ open the family â†’ Leave home. */
        MEMBER_LEAVES,
        /** ADMIN: Manage Family â†’ open the family â†’ member row â†’ Remove member. */
        ADMIN_REMOVES
    }

    /**
     * Which revocation path runs â€” {@code member_leave}, {@code admin_remove}, or
     ...
     * like UDIDs â€” you don't want a stray {@code -D} silently retargeting a phone. It
    ...
    private static String resolveCleanupMode() {
        ...
        if (cli != null && !cli.isBlank()) {
            logpoint("[Flow2LP] CLEANUP_MODE from -D: " + cli.trim());
    ...
    /**
     * Seeded so the choice is reproducible from the logged seed â€” an unseeded
    ...
    private RevokeMode pickRevokeMode() {
        ...
        switch (CLEANUP_MODE) {
            ...
            default -> {
                ...
                // like the pinned path working â€” say so instead of guessing quietly.
                System.err.println("[Flow2LP] Unrecognised CLEANUP_MODE='" + CLEANUP_MODE
                        + "' â€” expected member_leave | admin_remove | random. "
                ...
                reporter.log(Status.WARNING, "Unrecognised CLEANUP_MODE='" + CLEANUP_MODE
                        + "' â€” revocation path was randomised, not pinned.");
            }
        }

        RevokeMode picked = REVOKE_RANDOM.nextBoolean()
                ? RevokeMode.MEMBER_LEAVES : RevokeMode.ADMIN_REMOVES;
        logpoint("[Flow2LP] CLEANUP_MODE=" + CLEANUP_MODE + " â†’ " + picked
                + " (replay with -Dflow2.revokeSeed=" + REVOKE_SEED + ")");
        return picked;
    }

    /**
     * Revokes the member's access by ONE of the two paths â€” they are alternatives that
    ...
    private void refreshBystanderPhone(RevokeMode mode) {
        try {
            switch (mode) {
                // Member left â†’ the ADMIN's family list still lists them.
                case MEMBER_LEAVES -> adminSharingPage.refreshHomeDashboard();
                // Admin removed them â†’ the MEMBER's home still shows the shared devices.
